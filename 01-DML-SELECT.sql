...
SELECT first_name, last_name,
    CONCAT(first_name, CONCAT(' ', last_name))문자열연결함수,
    first_name ||' '|| last_name 문자열연결연산,
    INITCAP(first_name ||' '|| last_name) INITCAP -- 각 단어의 첫 글자 대문자
FROM employees;
...
SELECT '     Oracle     ',
    ...
    SUBSTR('Oracle Database', -8, 4),    -- 역인덱스 이용 부분 문자열 추출
...
SELECT sysdate FROM employees; -- employees 테이블로부터 받아오므로 employees 테이블 레코드 갯수만큼
...
SELECT
    ...
    NEXT_DAY(sysdate, '월'),               -- NLS_DATE_LANGUAGE의 설정에 따름
...
SELECT '2012-09-24 13:48:00',
    TO_DATE ('2012-09-24 13:48:00', 'YYYY-MM-DD HH24:MI:SS') 날짜변환
FROM dual;
...
SELECT 
    ...
    END AS 팀
FROM employees
ORDER BY 팀 ASC, department_id ASC;
